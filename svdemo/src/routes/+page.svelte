...
<div class="demo">
    <SC.Canvas>
        <SC.Mesh material={new THREE.MeshStandardMaterial({color: 0xffffff,emissive:0xffffff})} geometry={new THREE.SphereGeometry(10)} position={[0,0,0]}></SC.Mesh>
        ...
        {#each stars as star}
            {console.log(star)}
            <DecStar rightAscension={star[1]} declination={star[2]} distance={(1/star[3])*3.26} radius={0.1}></DecStar>
        {/each}
